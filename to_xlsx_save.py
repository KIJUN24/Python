...
def clear():
    global sum_order, order_total, order_bev, order_des, textarea, ent1, ent2
...
def add1(item1):
    ...
    sum_order += this_price1
    order_total.append(item1)
    order_bev[item1] += 1
    textarea.insert(tk.INSERT, item1 +" ")
    label5['text'] = "금액 : " + str(sum_order) + "원"
    # print(type(sum_order), type(this_price1))
...
def send():
    global order_total, order_bev, order_des, sum_order, ent1, ent2
    name = str(ent1.get())
    hp = str(ent2.get())
    print(name, hp)
    print(order_total)
    print(order_bev)
    print(order_des)
    print(str(sum_order) + "원")
    now_dt = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
    wb1 = load_workbook("test_order.xlsx")
    ws1 = wb1['order']
    ...
    sum_order])
    wb1.save("test_order.xlsx")
    clear()


def maker_one():
    wb2 = load_workbook("test_order.xlsx")
    ws2 = wb2['order']
    ws2.append(["날짜", "이름", "휴대폰 번호", "커피", "라떼", "스무디", "차", "케이크", "쿠키", "아이스크림", "타르트", "총금액"])
    wb2.save("test_order.xlsx")
...
def read_xlsx():
    df_rx = pd.read_excel("C:\\Users\\lkjun\\OneDrive\\바탕 화면\\PythonWorkspace\\python_study\\Python_practice\\test_order.xlsx",
    sheet_name = "order")
    df_rx.to_excel("C:\\Users\\lkjun\\OneDrive\\바탕 화면\\PythonWorkspace\\python_study\\Python_practice\\test_order.xlsx"
