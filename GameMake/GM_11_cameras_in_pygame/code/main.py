...
class CameraGroup(pygame.sprite.Group):
    ...
    def box_target_camera(self, target):
        ...
        if target.rect.left < self.camera_rect.left:    # 플레이어가 카메라 사각형보다 더 왼쪽에 있다는 의미
    ...
    def keyboard_control(self):     # 키보드를 이용하여 카메라 조작(w, a, s, d)
        keys = pygame.key.get_pressed()
        if keys[pygame.K_a]:
...
pygame.event.set_grab(True) # 마우스가 화면 밖으로 못 떠나게 하는 코드 --> 사용할 때 주의!!

# setup
camera_group = CameraGroup()
player = Player((640, 360), camera_group)

for i in range(20): # 20개의 tree 생성
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:    # ESC 누르면 게임 끄는 것 가능
                pygame.quit()
                sys.exit()
