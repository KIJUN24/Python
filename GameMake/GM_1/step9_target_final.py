...
cvs.create_line((round(wbar_x+wbar_w*1/2), wbar_y), (round(wbar_x+wbar_w*1/2), wbar_y+wbar_h), width = 2)
wind = 0  # -100 ~ 100
wind_ax_max = 0.5
wind_ax = wind_ax_max * wind * 1/100
bbar_mx = round(wbar_h * 1/10)
bbar_my = bbar_mx
if wind >= 0 :
    bbar_x = round(wbar_x+wbar_w*1/2) + bbar_mx
else :
    bbar_x = round(wbar_x+wbar_w*1/2) - bbar_mx
bbar_y = wbar_y + bbar_my
bbar_w = (wbar_w*1/2 - bbar_mx*2) * wind * 1/100
bbar_h = wbar_h - bbar_my*2
...
##################### Target #####################
t_r = 40
t_range_x = (round(win_w * 1/3), win_w - t_r)
...
t_img = Image.open("C:\\Users\\lkjun\\OneDrive\\바탕 화면\\PythonWorkspace\\python_study\\Python_practice\\GameMake\\GM_1\\target.png")
t_img = t_img.resize((t_r*2, t_r*2), Image.ANTIALIAS)
...
count = 0
c_box = cvs.create_text((round(cbox_x + cbox_w * 1/2), round(cbox_y + cbox_h * 1/2)), fill = "white", font = ("Arial", 30), text = count)
...
while True:
    ...
    if stage == 3:
        ...
        bomb = cvs.create_oval((bomb_cx - bomb_r, bomb_cy - bomb_r), (bomb_cx + bomb_r, bomb_cy + bomb_r), fill = "red")
        # target start
        dist = ((bomb_cx - t_cx) ** 2 + (bomb_cy - t_cy) ** 2) ** 0.5    # ** 0.5 : 루트
        if dist <= bomb_r+t_r :
            hit = True
        if bomb_cy >= top_h or hit == True :   # bomb and target
            stage = 1
            power = 0
            # wind
