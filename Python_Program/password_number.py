# Genetic Algorithm
# password를 알고 있을 때 가정.
# 1. 비밀번호의 길이
# 2. 똑같은 자리에 똑같은 숫자나 문자가 있는지

# 순서
# 1. 최초의 아이들 생성(아이들 : 비밀번호의 집합)
# 2. Fitness : 아이들 유전자 성능 측정(유전자 성능 점수 -> 점수가 높을수록 좋음.)
# 3. Compute Perfomance and Select Survivors : 똑똑한 아이를 선발하는 과정
# 4. Create Children : 살아남은 아이들끼리 교배
# 5. Mutation : 돌연변이 생성 -> 진화하는 과정.
