...
def menu_make():
    ...
    for num in range(0,):
        num += 1
    df = df_maker(col_num, num, sum)
    df.columns = ["이름", "전화번호", "커피", "라떼", "스무디", "차", "케이크", "쿠키", "아이스크림", "타르트", "총금액"]
    df.index = num
    print(df)
...
def send():
    ...
    wb = load_workbook("test_order.xlsx")
    ws = wb['order']
    ws.append([now_dt, name, hp, 
